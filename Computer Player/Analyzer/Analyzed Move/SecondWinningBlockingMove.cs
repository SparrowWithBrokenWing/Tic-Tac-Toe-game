...
public class SecondWinningBlockingMove : DefensiveMove
    {
        public SecondWinningBlockingMove(IMove move) : base(move)
        {
        }

    protected override bool Validate(IMove move)
    {
        var willThisMoveCompleteAPlayersLineSegmentWithLengthAsWinningCondition = (IMove move) =>
        {
            ...
            while (checkedMove < 8)
            {
                ...
                if (satisfiedCheckedMove == move.BoardStateBeforeBeingPlayed.PlayingBoard.WinningCondition - 1)
                {
        ...
        return base.Validate(move)
            && willThisMoveCompleteAPlayersLineSegmentWithLengthAsWinningCondition(move);
    }
}
