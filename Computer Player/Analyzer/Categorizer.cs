namespace ComputerPlayer
{
    ...
    public class ForkMoveTypeCategorizer : OffensiveMoveTypeCategorizer
    {
        ...
        public override IEnumerable<IMoveType> Categorize(IMove move, IMoveRetriever moveRetriever, IBoard playingBoard)
        {
            ...
            var check = async (
                Func<uint, uint> getNextFrontMoveRowIndex,
                Func<uint, uint> getNextFrontMoveColumnIndex,
                Func<uint, uint> getNextBehindMoveRowIndex,
                Func<uint, uint> getNextBehindMoveColumnIndex,
                ...
                ) =>
            {
                ...
                var checkPlayerofMove = (
                    uint limitation,
                    Func<uint, uint> getNextCheckedMoveRowIndex,
                    Func<uint, uint> getNextCheckedMoveColumnIndex,
            ...
            var checkVerticalLine = check(
                    (uint checkingMoveRowIndex) => checkingMoveRowIndex + 1,
                    (uint checkingMoveColumnIndex) => checkingMoveColumnIndex,
                    (uint checkingMoveRowIndex) => checkingMoveRowIndex - 1,
                    (uint checkingMoveColumnIndex) => checkingMoveColumnIndex,
                    cancellationToken)
                .ContinueWith(handleFinishedCheckTask);
            var checkHorizontalLine = check(
                    (uint checkingMoveRowIndex) => checkingMoveRowIndex,
                    (uint checkingMoveColumnIndex) => checkingMoveColumnIndex + 1,
                    (uint checkingMoveRowIndex) => checkingMoveRowIndex,
                    (uint checkingMoveColumnIndex) => checkingMoveColumnIndex - 1,
                    cancellationToken)
                .ContinueWith(handleFinishedCheckTask);
