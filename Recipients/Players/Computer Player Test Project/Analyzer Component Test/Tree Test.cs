...
namespace Computer_Player_Test_Project.Analyzer_Component_Test
{
    ...
    {
        [TestMethod]
        public void VerifyPredictedPreviousMoveRetrieverWorkAsExpected()
        {
            ...
            var playedMove1 = getNewMockMove(0, 0, mockOfPlayer.Object);
            addNewPlayedMove(playedMove1);
            var playedMove2 = getNewMockMove(1, 0, mockOfOpponent.Object);
            addNewPlayedMove(playedMove2);
            var playedMove3 = getNewMockMove(0, 1, mockOfPlayer.Object);
            addNewPlayedMove(playedMove3);
            var playedMove4 = getNewMockMove(0, 2, mockOfOpponent.Object);
            addNewPlayedMove(playedMove4);
            ...
            var node1 = treeFlowerFactory.Produce(1, 1, player);
            ((IPreviousMoveSpecifiableMove)node1).PreviousMove = lastPlayedMove;
            var node2 = treeFlowerFactory.Produce(2, 2, opponent);
            ((IPreviousMoveSpecifiableMove)node2).PreviousMove = node1;
            var node3 = treeFruitFactory.Produce(2, 1, player);
            ((IPreviousMoveSpecifiableMove)node3).PreviousMove = node2;
            var methodInfo = typeof(TreeImplement1
                <IAnalyzableTreeComponent,
                IAnalyzableTreeBranch,
                IAnalyzableTreeLeaf,
                IAnalyzableTreeFlower,
                IAnalyzableTreeFruit>).GetMethod("GetMoveRetriever", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance, null, new Type[] { typeof(IPreviousMoveAccessibleMove) }, null);
            var test = (IMoveRetriever)methodInfo.Invoke(tree, new object[] { node3 });

            var testData = new List<IMove>(test);
            var expectedMoves = new List<IMove>
            {
                playedMove1,
                playedMove2,
                playedMove3,
            ...
            mockOfIEqualityComparer
                ...
                .Returns<IMove, IMove>((first, second) =>
                {
            ...
            testData.SequenceEqual(expectedMoves, mockOfIEqualityComparer.Object);
        }
    }
}
